...
const STATE = {
  ...
  pagosYear: null, // año seleccionado en Pagos
};
...
const DATA = {
  ...
  /* ====== Mensualidades mock (año completo) ======
     id: "YYYY-MM", anio: 2025, mes: 1..12, pagada: bool, importe: CLP,
...
// Ayudas para meses y CLP (respetando formato local)
...
function renderPagosContent(){
  // Año por defecto = el más alto disponible
  if(!STATE.pagosYear){
    STATE.pagosYear = Math.max(...DATA.mensualidades.map(m=>m.anio));
  }
  const years = Array.from(new Set(DATA.mensualidades.map(m=>m.anio))).sort((a,b)=>b-a);

  // Mensualidades del año seleccionado
  const mensualidades = DATA.mensualidades
    .filter(m=>m.anio===STATE.pagosYear)
    .sort((a,b)=>a.mes-b.mes);

  const totalPagado    = mensualidades.filter(m=>m.pagada).reduce((s,m)=>s+m.importe,0);
  const totalPendiente = mensualidades.filter(m=>!m.pagada).reduce((s,m)=>s+m.importe,0);

  mount(`
    <div class="row" style="justify-content:space-between;flex-wrap:wrap">
      <div class="row" style="gap:.6rem">
        <h2 class="section-title" style="margin:0">Pagos</h2>
        <label class="subtle">Año</label>
        <select id="anio-select" class="input" style="max-width:140px">
          ${years.map(y=>`<option value="${y}" ${y===STATE.pagosYear?"selected":""}>${y}</option>`).join("")}
        </select>
      </div>
      <div class="row">
    ...
    <div class="grid">
      ${UI.CardStat("Total Pagado", clp(totalPagado))}
      ${UI.CardStat("Total Pendiente", clp(totalPendiente))}
    </div>

    <div class="card" style="margin-top:1rem;padding:0">
      <div class="row" style="padding:1rem;justify-content:space-between;gap:.6rem;flex-wrap:wrap">
        <div class="row" style="gap:.6rem">
          <label class="subtle"><input type="checkbox" id="sel-todos" /> Seleccionar todos los pendientes</label>
        </div>
        <div class="row" style="gap:.6rem">
          <button class="btn btn-secondary" id="btn-pagar-seleccion">Pagar seleccionados</button>
          <button class="btn btn-primary"   id="btn-pagar-anio">Pagar año completo</button>
        </div>
      </div>

      <div id="pagos-card" style="padding:0">
        <table class="table">
          <thead>
            <tr>
          ...
          <tbody>
            ${mensualidades.map(m=>`
              <tr data-id="${m.id}">
                <td>
                  ${m.pagada ? "" : `<input type="checkbox" class="chk" value="${m.id}" />`}
                </td>
                <td>${mesNombre(m.mes)} / ${m.anio}</td>
                <td>${fmtDate(m.fecha_vencimiento)}</td>
                <td>${clp(m.importe)}</td>
                <td>
                  ${m.pagada 
                    ? `<span class="tag graded">Pagada</span>`
  ...
  // Eventos UI
  $("#anio-select").addEventListener("change", (e)=>{ STATE.pagosYear = Number(e.target.value); renderPagosContent(); });
  ...
  // Selección múltiple
  const checks = $$("#pagos-card .chk");
  $("#sel-todos").addEventListener("change", (e)=>{ checks.forEach(c=>{ c.checked = e.target.checked; }); });

  // Pagar seleccionados
  $("#btn-pagar-seleccion").addEventListener("click", ()=>{
    const ids = checks.filter(c=>c.checked).map(c=>c.value);
    if(!ids.length){ 
      UI.Modal.show("Sin selección", `<div class="subtle">No seleccionaste mensualidades pendientes.</div>`); 
      return; 
    }
    confirmarPago(ids);
  });

  // Pagar año completo (todos los pendientes del año)
  $("#btn-pagar-anio").addEventListener("click", ()=>{
    const ids = mensualidades.filter(m=>!m.pagada).map(m=>m.id);
    if(!ids.length){
      UI.Modal.show("Todo al día", `<div class="subtle">No hay mensualidades pendientes en ${STATE.pagosYear}.</div>`);
      return;
    }
    confirmarPago(ids);
  });

  // PDF por año con resumen + tabla actual
  $("#btn-pdf-pagos").addEventListener("click", async ()=>{
    const perfil = await api.getPerfil();
    const academicSummary = await buildAcademicSummaryHTML();
    const pagosSummary = `
      <div class="summary">
        <div class="chip"><strong>Año:</strong> ${STATE.pagosYear}</div>
        <div class="chip"><strong>Total pagado:</strong> ${clp(totalPagado)}</div>
        <div class="chip"><strong>Total pendiente:</strong> ${clp(totalPendiente)}</div>
      </div>
    `;
    const tableHTML = $("#pagos-card").outerHTML;
    exportToPDFWithHeader(
      `pagos_${STATE.pagosYear}.pdf`,
      "Informe de Pagos",
      tableHTML,
      { preHTML: `${academicSummary}${pagosSummary}`, student: perfil, logoUrl: getLogoUrl(), photoUrl: getStudentPhotoUrl(), watermarkText:"INTRANET" }
    );
  });

  // Confirmación y simulación de pago en front (marca como pagadas en el mock)
  function confirmarPago(ids){
    const sel = DATA.mensualidades.filter(m=>ids.includes(m.id));
    const total = sel.reduce((s,m)=>s+m.importe,0);
    UI.Modal.show("Confirmar pago", `
      <div class="subtle">Vas a pagar <strong>${ids.length}</strong> mensualidad(es) del año ${STATE.pagosYear}:</div>
      <ul>${sel.map(m=>`<li>${mesNombre(m.mes)} / ${m.anio} — ${clp(m.importe)}</li>`).join("")}</ul>
      <div class="subtle" style="margin-top:.4rem"><strong>Total:</strong> ${clp(total)}</div>
      <div class="row" style="justify-content:flex-end;margin-top:.8rem">
        <button class="btn btn-secondary" id="btn-cancel">Cancelar</button>
        <button class="btn btn-primary" id="btn-confirm">Confirmar pago</button>
      </div>
    `);
    $("#btn-cancel").addEventListener("click", UI.Modal.hide);
    $("#btn-confirm").addEventListener("click", ()=>{
      DATA.mensualidades.forEach(m=>{ if(ids.includes(m.id)) m.pagada = true; });
      UI.Modal.hide();
      renderPagosContent();
    });
  }
